public static String solve ( String string ) {
  ...
  while ( n > 0 ) {
    ...
    int tmpN = flag ? use . get ( a . length - 1 ) : Math . max ( use . size ( ) , 1 ) ;
    int diff = needs . get ( tmpN ) - needs . get ( a [ 0 ] ) ;
    int tmpC = n / diff ;
    if ( flag ) {
      base = base . substring ( 0 , base . length ( ) - tmpC ) + String . valueOf ( tmpN ) * tmpC ;
    }
    else {
      base = base . substring ( 0 , index ) + String . valueOf ( tmpN ) * tmpC + base . substring ( index + tmpC ) ;
      index += tmpC ;
    }
    n -= diff * tmpC ;
  }
