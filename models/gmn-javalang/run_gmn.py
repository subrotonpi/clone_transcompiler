import sys
sys.path.append('~/PycharmProjects/code-clone-multilingual/experimental/')
gnn_save_dir = '/home/egk204/PycharmProjects/code-clone-multilingual/experimental/gnn_model/saved/'
import torch
...
from experimental.gnn_model.createclone_main import createast,creategmndata,createseparategraph
import models

parser = argparse.ArgumentParser()
parser.add_argument("--cuda", default=False)
parser.add_argument("--storage", default='scb')
...
for epoch in epochs:# without batching
    ...
    devfile=open(gnn_save_dir+args.graphmode+'_dev_epoch_'+str(epoch+1),mode='w')
    for res in devresults:
        devfile.write(str(res)+'\n')
    devfile.close()
    testresults=test(testdata)
    resfile=open(gnn_save_dir+args.graphmode+'_epoch_'+str(epoch+1),mode='w')
    for res in testresults:
        resfile.write(str(res)+'\n')
    resfile.close()
    
    torch.save(model,gnn_save_dir+'model_gmn'+str(epoch+1))
