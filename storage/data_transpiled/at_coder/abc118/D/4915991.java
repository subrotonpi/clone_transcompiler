public static String solve ( String string ) {
  ...
  if ( a . contains ( 2 ) && ( a . contains ( 3 ) || a . contains ( 5 ) ) ) {
    a [ a . length - 1 ] = 2 ;
  }
  ...
  while ( n > 0 ) {
    ...
    for ( Integer _b : b ) {
      if ( needs . get ( _b ) <= needs . get ( b . get ( 0 ) ) ) {
        use . add ( _b ) ;
      }
    }
    if ( use . size ( ) == 1 ) {
    ...
    boolean flag = use . size ( ) == b . get ( 0 ) ;
    int tmpN = flag ? use . get ( use . size ( ) - 1 ) : use . size ( ) ;
    int tmpC = n / ( needs . get ( tmpN ) - needs . get ( b . get ( 0 ) ) ) ;
    if ( flag ) {
      base = base . substring ( 0 , base . length ( ) - tmpC ) + String . valueOf ( tmp @ @